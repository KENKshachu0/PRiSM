...
test("live grouping uses active sessions, preserves multiple plans and distinguishes unknown estimates", () => {
  ...
  const player: LivePlayer = { playerId: "p", displayName: "玩家", status: "active", walletTotal: centsOf(50), estimatedTotal: 40,
  ...
  expect(liveBilling({ ...player, walletTotal: centsOf(6) }).status).toBe("余额不足");
